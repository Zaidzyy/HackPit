...
- **C2-framework install + the persistence evasion/OPSEC half** (build #4) — Sliver, Empire and weevely are catalogued **reference-only** (not installed); their install and C2 wiring, and the operator-side evasion/OPSEC describe channel for persistence, are deferred to build #4. The TA0003 enrich shipped the knowledge, catalog and blue/describe layers only (Part III).
