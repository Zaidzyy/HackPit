...
# ---- AUTONOMY BUILD (2026-08-15): egress, auto-runner modes, scheduler, RoE wall, continuous hunting, decision queue ----
run_test test_egress_safety.py "EGRESS CONTROL (rotating source IP + identify header; grants nothing -- every gate still fires on the rewritten argv; engagement-mode only; the honest 'went direct' paths; a pool URL is a CREDENTIAL held only by egress_config and masked in every note; rotation wraps/skips-banned/exhausts; the pool never lands on the browser-facing record; AND container-level egress -- an unmapped tool (curl/python) with no native flag still returns the picked URL so the docker-exec proxy ENV routes it, credential OFF the argv)"
run_test test_autotier_safety.py "AUTO-RUNNER TIER CLASSIFIER (the passive allowlist is CLOSED, cross-checked against the REAL invokable surface set -- discover/jsrecon/nuclei, recon only when passive, smuggle/cache only at an EXPLICIT stage=detect; everything else incl. any unknown/new surface is exploitation; repeater + 'ask' are human_only always; a flagged command is exploitation, an unflagged real one passive; None/empty/kind-less proposals FAIL SAFE to exploitation)"
run_test test_autorun_safety.py "AUTO-RUNNER DECIDE + FIRE (manual fires NOTHING; assisted fires ONLY passive and QUEUES exploitation + human_only; full fires passive+exploitation but the repeater/'ask' still queue; an unknown mode fails safe to manual; THE TEETH -- a simulated assisted loop over a mixed batch fires only the passive surfaces; fire routes surface via _run_surface and command via the executor with approved+dangerous_ack, and appends -- never rewrites -- the secrets-stripped audit)"
run_test test_autoloop_safety.py "AUTO-RUNNER SCHEDULER (DEFAULT OFF -- this daemon has hands; only assisted/full engagements WITH a session are stepped, a manual one never; disabling halts the tick immediately -- the mid-tick kill-switch; at most one step per engagement per tick and a step error is contained to its own engagement)"
run_test test_roe_wall_safety.py "MODE-3 RoE WALL + BUDGET (governance.permits reads the RoE -- excluded_actions case-insensitive deny + time_windows blackout incl. overnight/malformed-fail-closed, and DESCRIBES only; autorun.permitted_to_fire enforces before EVERY autonomous fire -- a per-engagement budget, the RoE deny/blackout, and an unreadable RoE FAILS CLOSED; THE TEETH -- step_session downgrades a mode-allowed but RoE-forbidden exploitation to a QUEUE, never fires it)"
run_test test_watch_diff.py "CONTINUOUS HUNTING (snapshot the engagement's discovered assets -> diff vs the previous snapshot -> new-asset alert; the FIRST check is a silent baseline; a new asset diffs + alerts; an unchanged snapshot stays silent; READ-ONLY -- watch imports only read/state paths and no execution surface, by AST)"
run_test test_decisionqueue_safety.py "DECISION QUEUE (a held action round-trips WITH its full proposal so it can be re-fired; the same held action queued each tick DEDUPS to one row; mark moves it out of pending and the action can re-queue; APPROVE fires the held proposal through autorun.fire and SKIP dismisses WITHOUT firing; approving an unknown/already-decided item is refused 404/409)"
