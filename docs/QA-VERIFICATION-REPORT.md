...
~120 forced recalcs per second, forever, for a value that only changes when the operator picks
a new accent. It is now read once and cached, refreshed by a `MutationObserver` on the `style`
attribute of `<html>` (which is exactly how `TopBar.setAccent` applies it), so the live reskin
is still instant and steady-state cost drops to zero. No visual change.

**Deliberately NOT done.** The remaining cost is the per-frame canvas draw itself, and every
way to reduce it (lower frame rate, coarser grid, fewer dots) changes a signature visual. The
`react-hooks/set-state-in-effect` errors (LOGGED-1) were also left alone: they fire once on
mount, not per frame, so they are not this bug — and `frontend/AGENTS.md` records that
auto-fixing them risks the reveal/data-load animations.

**Not measured.** No before/after frame-time numbers: the Chrome extension was not connected
and the repo has no headless browser driver, so this is static analysis (call counts per frame),
not a runtime profile. Whether the residual stutter is now gone is **unconfirmed**.
