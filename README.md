...
> The knowledge sources the KB is built from — course notes, write-ups, cloned repos, PDFs, cheat sheets — are kept **local and git-ignored, not shared here contact me for the complete KB for full access to this tool**. **a fresh clone starts with an empty knowledge base until you build one or get it from the author.** 
