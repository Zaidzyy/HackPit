...
def test_non_allowlisted_command_is_flagged() -> None:
    resp = '{"done": false, "command": "nikto", "args": ["-h", "hackpit-lab-target"], "rationale": "scan"}'
    with _LLM(resp):
        out = O.propose_next(PLAN, [], {}, [])
    p = out["proposal"]
    assert p is not None and p["gate_ok"] is False, "a non-allowlisted command must be flagged"
    assert "allowlist" in p["gate_reason"].lower()
    print("  non-allowlisted command proposal is flagged: PASS")


def test_metachar_arg_is_flagged() -> None:
    resp = (
        '{"done": false, "command": "curl", "args": ["http://hackpit-lab-target/; id"], '
        '"rationale": "x"}'
    )
    with _LLM(resp):
        out = O.propose_next(PLAN, [], {}, [])
    p = out["proposal"]
    assert p is not None and p["gate_ok"] is False, "a shell metachar must be flagged"
    print("  shell-metachar arg proposal is flagged: PASS")


def test_precheck_direct() -> None:
    ok, _ = O.precheck("nmap", ["-sV", "hackpit-lab-target"])
    assert ok, "plain lab nmap must pass"
    ok, reason = O.precheck("nmap", ["--script", "vuln", "hackpit-lab-target"])
    assert not ok, "nmap scripting must be blocked (matches the frozen allowlist)"
    ok, reason = O.precheck("bash", ["-c", "id"])
    assert not ok and "allowlist" in reason, "non-allowlisted must be blocked"
    print("  precheck mirrors the real M1 gates: PASS")
...
if __name__ == "__main__":
    ...
    test_non_allowlisted_command_is_flagged()
    test_metachar_arg_is_flagged()
