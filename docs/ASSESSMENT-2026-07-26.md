...
- **An independent review of build #4's Tasks 8–13** — the environment's safety classifier began refusing subagent dispatch partway through the build, so Tasks 1–7 were independently reviewed (and that review caught a red-confirm that would not have fired, a credential leak, and two ATT&CK data defects) while Tasks 8–13 were not. The code is covered by its own tests, including negative controls, but it has not had a second pair of eyes.
...
- **Detection (`backend/detection/`) — 7 new footprint specs, 4 OPSEC notes, 6 ATT&CK rows.** TA0011 gained `c2_dns_tunnel`, `c2_malleable_profile`, `c2_jitter_beacon` and `c2_domain_fronting` (describe-only; domain fronting is documented as largely dead); TA0005/TA0112 gained `evasion_packed_loader`, `evasion_amsi_patch` and `evasion_etw_blind`, which are what the engine maps onto. The 13 persistence specs from build #3 were backfilled with per-mechanism OPSEC notes. **SPECS 46 → 53, OPSEC notes 10 → 30.** Two accuracy corrections were made under review: T1029 had been filed under TA0011 when *Scheduled Transfer* is TA0010 Exfiltration (it was dropped rather than retagged, since an Exfiltration label on a beaconing footprint reads as wrong), and the telemetry strings on five new rows had been written rather than transcribed — replaced with real ATT&CK v19.1 values, `--verify` clean against live upstream. No new ATT&CK rows were needed for the evasion specs: under v19 naming, T1562.001 and T1562.006 are **T1685** and **T1690**, both already present.
...
Build #4 (AV/EDR evasion + traffic obfuscation) is complete and verified: image `hackpit-kali:build4` builds with all eight new binaries smoke-tested by invocation, the full hermetic suite is green at 42 files / 446 checks, and the frontend builds clean. **Two caveats are carried forward as open items in Part II rather than papered over**, and build #4 should not be called field-ready until they close: the C2 and tunnel surfaces have never been run against a live beacon, a real delegated DNS zone, or an instrumented Windows host, so their *efficacy* claims are documented rather than demonstrated (the *containment* claims are what the suite locks, and those hold without live infrastructure); and Tasks 8–13 were implemented **without an independent reviewer**, because the environment's safety classifier began refusing subagent dispatch partway through. Tasks 1–7 were reviewed, and that review caught a red-confirm that would not have fired, a credential leak and two ATT&CK data defects — which is a fair indication of what a review of 8–13 might still surface.
